...
import {
  ...
  Ban, LockOpen, Package, Shield, Star, Download, ShoppingBag,
} from "lucide-react"
...
export default function SuperAdminDashboard() {
  ...
  const { isLoggedIn, adminInfo, users, totalUsers, admins, bloggers, totalBloggers,
          superAdmins, selectedUser, selectedAdmin, selectedBlogger,
          adminReviews, adminReviewsAvg, adminReviewsCount,
          allOrders, totalOrders,
          loading, error, actionResult }
    = useSelector(s => s.superAdmin)

  const [tab, setTab] = useState("users")   // users | admins | bloggers | superadmins | orders
  ...
  const load = useCallback(() => {
    ...
    if (tab === "orders")      dispatch(fetchAllOrders({ page, limit: LIMIT }))
  ...
  const viewAdmin   = (id) => { dispatch(fetchAdminById(id)); dispatch(fetchAdminReviews(id)); setDetailModal("admin") }
    ...
    <div style={{ minHeight: "100vh", background: "#f8f9fa", fontFamily: "'Inter',sans-serif" }}>
      ...
      <div style={{ maxWidth: 1200, margin: "0 auto", padding: "24px 16px" }}>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit,minmax(200px,1fr))", gap: 16, marginBottom: 28 }}>
          {[
            ...
            { icon: <Package size={22} color={RED} />, label: "Sifarişlər",   value: totalOrders,                tab: "orders"      },
          ].map(c => (
          ...
          {/* Komisya Paneli — ayrı səhifəyə keçid */}
          <div
          ...
          >
            ...
            <div>
              <div style={{ fontSize: 15, fontWeight: 800 }}>Komisya Paneli</div>
              <div style={{ fontSize: 11, opacity: 0.85, marginTop: 2 }}>Balanslar · Demo webhook</div>
            </div>
          </div>

          {/* Dəstək Chat — alıcı söhbətlərini izlə */}
          <div
          ...
          >
            ...
            <div>
              <div style={{ fontSize: 15, fontWeight: 800 }}>Dəstək Chat</div>
              <div style={{ fontSize: 11, opacity: 0.85, marginTop: 2 }}>Alıcı söhbətləri · Real-time</div>
            </div>
          </div>
        </div>
          ...
          {loading ? (
                      ...
                      <td style={{ padding: "12px 16px", fontSize: 13, color: "#555" }}>
                        ...
                        {a.numReviews > 0 && (
                          <div style={{ display: "flex", alignItems: "center", gap: 4, marginTop: 4 }}>
                            ...
                            <span style={{ fontSize: 11, fontWeight: 700, color: a.avgRating >= 4 ? "#16a34a" : a.avgRating >= 3 ? "#d97706" : "#dc2626" }}>
                              {Number(a.avgRating).toFixed(1)}
                            </span>
                            <span style={{ fontSize: 10, color: "#aaa" }}>({a.numReviews})</span>
                            <span style={{ fontSize: 10, fontWeight: 700, color: a.avgRating >= 4 ? "#16a34a" : a.avgRating >= 3 ? "#d97706" : "#dc2626" }}>
                              {a.avgRating >= 4 ? "Yaxşı" : a.avgRating >= 3 ? "Orta" : "Zəif"}
                            </span>
                          </div>
                        )}
                      </td>
            ...
            /* ── ORDERS TABLE ── */
              ...
              <table style={{ width: "100%", borderCollapse: "collapse" }}>
                <thead>
                  <tr style={{ background: "#f8f9fa", fontSize: 12, color: "#888" }}>
                    {["Sifariş №", "Alıcı", "Məhsullar", "Məbləğ", "Status", "Tarix", "Çek"].map(h => (
                      <th key={h} style={{ padding: "12px 16px", textAlign: "left", fontWeight: 700 }}>{h}</th>
                    ))}
                  </tr>
                </thead>
                <tbody>
                  {allOrders.length === 0 ? (
                    <tr><td colSpan={7} style={{ padding: 40, textAlign: "center", color: "#bbb" }}>Sifariş tapılmadı</td></tr>
                  ) : allOrders.map(o => (
                    <tr key={o._id} style={{ borderTop: "1px solid #f0f0f0" }}>
                      <td style={{ padding: "12px 16px", fontSize: 12, fontFamily: "monospace", color: "#555" }}>
                        #{o._id.slice(-8).toUpperCase()}
                      </td>
                      <td style={{ padding: "12px 16px" }}>
                        <div style={{ fontWeight: 600, fontSize: 13, color: "#111" }}>{o.user?.name || "—"}</div>
                        <div style={{ fontSize: 11, color: "#aaa" }}>{o.user?.email || ""}</div>
                      </td>
                      <td style={{ padding: "12px 16px", fontSize: 12, color: "#555" }}>
                        {o.orderItems?.slice(0, 2).map((it, i) => (
                          <div key={i}>{it.name} ×{it.quantity}</div>
                        ))}
                        {o.orderItems?.length > 2 && <div style={{ color: "#aaa" }}>+{o.orderItems.length - 2} daha...</div>}
                      </td>
                      ...
                      <td style={{ padding: "12px 16px" }}>
                        {o.receiptUrl ? (
                          <a href={o.receiptUrl} target="_blank" rel="noreferrer"
                            style={{ display: "flex", alignItems: "center", gap: 4, fontSize: 12, fontWeight: 700, color: RED, textDecoration: "none" }}>
                            <Download size={13} /> Çek
                          </a>
      ...
      {detailModal === "admin" && selectedAdmin && (
        <Modal title="Satıcı Məlumatları" onClose={() => { setDetailModal(null); dispatch(clearSelected()) }}>
          ...
          {/* Rəylər bölməsi */}
          {adminReviewsCount > 0 || adminReviews.length > 0 ? (
            <div style={{ marginTop: 20 }}>
              <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 12, paddingTop: 16, borderTop: "1px solid #f0f0f0" }}>
                <Star size={15} fill="#FBBF24" color="#FBBF24" />
                <span style={{ fontWeight: 800, fontSize: 14, color: "#111" }}>
                  Mağaza Rəyləri — {Number(adminReviewsAvg).toFixed(1)} / 5
                </span>
                <span style={{ fontSize: 12, color: "#aaa" }}>({adminReviewsCount} rəy)</span>
                <span style={{ fontSize: 11, fontWeight: 700, color: adminReviewsAvg >= 4 ? "#16a34a" : adminReviewsAvg >= 3 ? "#d97706" : "#dc2626", marginLeft: "auto" }}>
                  {adminReviewsAvg >= 4 ? "Yaxşı mağaza" : adminReviewsAvg >= 3 ? "Orta mağaza" : "Zəif mağaza"}
                </span>
              </div>
              <div style={{ maxHeight: 220, overflowY: "auto", display: "flex", flexDirection: "column", gap: 10 }}>
                {adminReviews.map(rv => (
                  <div key={rv._id} style={{ background: "#f8f9fa", borderRadius: 10, padding: "10px 14px" }}>
                    <div style={{ display: "flex", justifyContent: "space-between", marginBottom: 4 }}>
                      <span style={{ fontWeight: 700, fontSize: 13, color: "#111" }}>{rv.userName}</span>
                      <div style={{ display: "flex", alignItems: "center", gap: 4 }}>
                        {[1,2,3,4,5].map(s => (
                          <Star key={s} size={11} fill={s <= rv.rating ? "#FBBF24" : "none"} color={s <= rv.rating ? "#FBBF24" : "#ddd"} />
                        ))}
                        <span style={{ fontSize: 11, color: "#aaa", marginLeft: 4 }}>{new Date(rv.createdAt).toLocaleDateString("az-AZ")}</span>
                      </div>
                    </div>
                    {rv.comment && <p style={{ margin: 0, fontSize: 13, color: "#555" }}>{rv.comment}</p>}
                  </div>
                ))}
              </div>
            </div>
          ) : (
            <p style={{ marginTop: 16, fontSize: 13, color: "#bbb", textAlign: "center" }}>Hələ rəy yoxdur</p>
          )}
